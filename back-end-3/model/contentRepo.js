...
class ContentRepo
{
  ...
  query_get_content_5(numOfLoadedContents)
  {
  ...
  {
    return `SELECT * FROM topic ORDER BY id DESC WHERE (title LIKE '%${nKeyWord}%' OR description LIKE '%${nKeyWord}%')`;
  }

  query_get_partial_by_search(nKeyWord, nLoadedContents)
  {
    return `SELECT * from topic ORDER BY id DESC WHERE (title LIKE '%"${nKeyWord}"%' OR description LIKE '%"${nKeyWord}"% LIMIT ${nLoadedContents},5')`;
  }
